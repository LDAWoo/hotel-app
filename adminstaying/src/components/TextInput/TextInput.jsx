...
function TextInput({ className, label, classLabel, classBorder, classInput, classToolTip, error, icon, classIcon, sizeIcon, copy, classCopy, iconCopy, sizeIconCopy, currency, classCurrency, tooltip, placement, placeholder, onClickCopy, ...props }) {
  ...
  return (
    <div className={`${className}`}>
      ...
      <div className={`relative w-full rounded-lg duration-200 outline-none   ${!classBorder ? `border-[2px] ${active ? "border-hotel-100 " : `${error ? "border-red-500" : "border-gray-200 dark:border-primary-500"}`}` : classBorder}`}>
        <input
          className={`${!classInput ? `bg-transparent hover:bg-transparent rounded-lg w-full h-[35px] pt-1 pb-1 pr-[10px] outline-none text-primary-700 placeholder:text-primary-100 dark:placeholder:text-primary-50 dark:text-white font-normal text-[14px] ${icon ? "pl-[48px]" : "pl-[10px]"}` : classInput} `}
