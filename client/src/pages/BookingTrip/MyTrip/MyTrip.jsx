                                    ...
                                    {data.map((item, index) => (
                                        <div key={index}>
                                            <div className="flex flex-col gap-1">
                                                <Title title={`${item?.address}`} fontBold extraLarge9 nowrap={false} />
                                                <Title title={`${format(new Date(), 'dd LLLL yyyy', {locale})} - ${format(new Date(), 'dd LLLL yyyy', {locale})}`} xxl nowrap={false} />
                                            </div>

                                            <Link to={`${routesConfig.mybooking}?bid=${item?.bookingId}`} target="_blank" className="relative z-0 mt-4 flex w-full rounded-md border border-gray-200 p-6 duration-200 shadow-[0_2px_8px_0_rgba(26,26,26,0.16)] hover:shadow-[0_2px_16px_0_rgba(26,26,26,0.24)]">
