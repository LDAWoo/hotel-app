    ...
    <div>
      {HotelPriceData.map((price, index) => (
        <div key={index} className='flex flex-col gap-2'>
          {price?.title && (
            <Title title={price?.title} fontBold nowrap={false} />
          )}
          {price?.type == "number" && (
            <div>
              {price?.data.map((item, index) => (
                <div
                  key={index}
                  className='flex flex-col gap-2 dark:text-primary-50'
                >
                  <Title title={item?.name} xl fontMedium />
                  <div
                    className={`flex relative border-[2px] rounded-sm duration-300 dark:text-white ${
                      active
                        ? "border-hotel-75"
            ...
            <div>
              {value > 0 && (
                <div className='flex flex-col gap-2 pl-2 pb-2'>
                  <div>
                    <Title title={`${price?.commissionRate}%`} /> Staying.com
                    commission
                  </div>
                  ...
                  <p className='mt-5'>
                    <MoneyFormatStaying price={totalPrice} /> Your earnings
                    (including taxes)
                  </p>
                </div>
              )}
