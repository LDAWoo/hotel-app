...
const ComponentPhotoHost = () => {
  const { onOpen } = useRegisterAIImageModal();
  const { photos, setPhotos } = useRegisterPhoto();
  ...
  return (
    <div className={`relative w-full p-4 border dark:border-primary-500 z-50 ${isDragging ? "border-hotel-200 bg-hotel-25 dark:bg-primary-600" : ""}`} onDragEnter={onDragEnter} onDrop={onDrop} onDragLeave={onDragLeave}>
      ...
      <div className="flex flex-col gap-2">
        <div className="font-medium inline dark:text-white text-[14px]">
          Upload at least 5 photos of your property. <span className="font-normal inline">The more you upload, the more likely you are to get bookings. You can add more later.</span>
        </div>
        ...
        <div className="basis-[222px] text-center border-none flex p-0 rounded-[3px] relative">
          <div className="p-4 bg-transparent border-[2px] border-dashed border-primary-100 dark:border-primary-200 box-border flex flex-col justify-around absolute top-0 bottom-0 w-full">
            ...
            <div className="w-auto">
              <Button className="w-auto justify-center p-2" fontMedium border icon={PiCameraLight} size={22} xl title="Upload photos" onClick={handleUploadPhoto} />
            </div>

            <Title title="jpg,jpeg or png, maximum 20MB each" xl colorTitle="dark:text-primary-50" />
          </div>
        </div>
        ...
        <div>
          <div className="flex flex-col text-[14px] gap-2 pt-[10px] text-primary-300 dark:text-primary-50 pointer-events-none">
            ...
            <span>
              Choose a <b className="dark:text-white">main photo</b> that will make a good first impression.
            </span>
            <span>Click and drag the photos to arrange them in the order you would like guests to see them.</span>
          </div>

          <div className="flex w-full pb-10 text-[14px]">
            <span>Bạn không có hình ảnh?</span>
            <div>
              <Button title="AI Image" className="text-hotel-50 hover:text-hotel-100 cursor-pointer" fontMedium onClick={handleShowPhotoModal} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-[calc(100%_+_2px)]">
            ...
            {images.length > 0 && <DragImage data={images} onClick={handleDeleteImage} images={photos} setImages={setPhotos} />}
          </div>
        </div>
      </div>
    </div>
